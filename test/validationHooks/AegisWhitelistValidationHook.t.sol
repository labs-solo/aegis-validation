...
contract AegisWhitelistValidationHookTest is Test {
    ...
    function _mintTierOne(address minter) private {
        vm.deal(minter, MINT_PRICE);
        vm.prank(minter);
        hook.mintTierOne{value: MINT_PRICE}(address(0));
    }

    function _mintTierTwo(address minter) private {
        vm.deal(minter, MINT_PRICE);
        vm.prank(minter);
        hook.mintTierTwo{value: MINT_PRICE}(address(0));
    }

    function _mintTierThree(address minter) private {
        vm.deal(minter, MINT_PRICE);
        vm.prank(minter);
        hook.mintTierThree{value: MINT_PRICE}(address(0));
    }
